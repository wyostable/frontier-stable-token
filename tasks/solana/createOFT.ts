...
import { TOKEN_PROGRAM_ID, createMultisig as createMultisigSplToken } from '@solana/spl-token'
...
import { checkMultisigSigners } from './multisig'
...
task('lz:oft:solana:create', 'Mints new SPL Token and creates new OFT Store account')
    ...
    .setAction(
        async ({
            ...
            const { connection, umi, umiWalletKeyPair, umiWalletSigner, keypair } = await deriveConnection(eid)
            ...
            if (additionalMintersAsStrings) {
                ...
                mintAuthorityPublicKey = await createMultisigSplToken(
                    connection,
                    keypair,
